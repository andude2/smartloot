...
local function bindLiveStats()
    mq.bind("/sl_stats", function(action)
        ...
        elseif action == "toggle" then
            ...
            util.printSmartLoot("Live stats window " .. (isVisible and "shown" or "hidden"), isVisible and "success" or "warning")
        ...
        elseif action == "compact" then
            ...
            util.printSmartLoot("Live stats compact mode " .. (not config.compactMode and "enabled" or "disabled"), "info")
        else
...
local function bindStatusCommands()
    ...
    mq.bind("/sl_engine_status", function()
        ...
        util.printSmartLoot("Current Corpse: " .. (state.currentCorpseID > 0 and tostring(state.currentCorpseID) or "None"), "info")
    ...
    mq.bind("/sl_debug", function(action, level)
        ...
        if action:lower() == "level" then
            if not level then
                ...
                util.printSmartLoot("Current debug level: " .. status.debugLevelName .. " (" .. status.debugLevel .. ")", "info")
...
local function bindTempRuleCommands()
    mq.bind("/sl_addtemp", function(...)
        ...
        if success then
            if rule == "KeepIfFewerThan" then
                util.printSmartLoot("Added temporary rule: " .. itemName .. " -> " .. rule .. " (" .. threshold .. ")", "success")
            else
    ...
    mq.bind("/sl_listtemp", function()
        ...
        for _, rule in ipairs(rules) do
            ...
            if displayRule == "KeepIfFewerThan" then
                util.printSmartLoot("  " .. rule.itemName .. " -> " .. displayRule .. " (" .. threshold .. ") [Added: " .. (rule.addedAt or "unknown") .. "]", "info")
            else
                util.printSmartLoot("  " .. rule.itemName .. " -> " .. displayRule .. " [Added: " .. (rule.addedAt or "unknown") .. "]", "info")
            end
        end
    end)
    ...
    mq.bind("/sl_afkfarm", function(action)
        ...
        elseif action:lower() == "status" then
            ...
            util.printSmartLoot("AFK Farming Mode: " .. (isActive and "ACTIVE" or "INACTIVE"), isActive and "success" or "warning")
            util.printSmartLoot("Temporary Rules: " .. count, "info")
